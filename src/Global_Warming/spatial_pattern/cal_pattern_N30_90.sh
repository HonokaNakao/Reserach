...
# MIROC6 ensemblemean & 50例とHadCRUT5 ensemblemean & 200例の空間相関を算出するプログラム (北緯32.5-87.5度)
#----------------------------------------------------------------------------------------------------------------------------------------------

mon=$1                                        # mon=1~12
DTOP=ensm_trend_1960_2014
DEND=mon

cd /home/honoka/work/sppattern/data/trend/         # トレンドデータの在り処に移動 

cat << EOF > cal_pattern_N30_90.f90
PROGRAM cal_sppattern_MIROC_HadCRUT

! MIROC6 ensemblemean & 50例とHadCRUT5 ensemblemean & 200例の空間相関を算出するプログラム
! gfortran -fconvert=big-endian cal_pattern_N30_90.f90
!----------------------------------------------------------------------------------------------
...
!! 入力ファイル名を指定               123456789-123456789-123456789-123456789-123456789-1
      CHARACTER ( LEN = 200 ) :: IFILE = 'ifnmMIROC_${mon}.txt'
      CHARACTER ( LEN = 200 ) :: IFILEHad ='HadCRUT5_${DTOP}_${mon}_${DEND}'                    ! input file name : HadCRUT5 ensemble data
...
! 欠損値以外のデータ総数
      numdt = 0.0
      DO k = 1792, KMAX                                       ! 北半球 (北緯32.5-87.5度)
...
! 平均値
      DO k = 1792, KMAX                                       ! 北半球 (北緯32.5-87.5度)
         IF ( trendHades(k) .NE. vmiss ) THEN	 
            sumtrendHades = sumtrendHades + trendHades(k)
         END IF
      END DO
      ...
      DO n = 1, ndata
         DO k = 1792, KMAX                                    ! 北半球 (北緯32.5-87.5度)
            IF ( trendMIROC(k,n) .NE. vmiss ) THEN	 
               sumtrendMIROC(n) = sumtrendMIROC(n) + trendMIROC(k,n)
            END IF
         END DO
      END DO
...
# コンパイル & 出力ファイル指定
gfortran -fconvert=big-endian ./cal_pattern_N30_90.f90;./a.out > sppattern_Hades-MIROC_${mon}_N30_90.txt

mv ./cal_pattern_N30_90.f90 /home/honoka/work/sppattern/src/
mv ./sppattern_Hades-MIROC_${mon}_N30_90.txt /home/honoka/work/sppattern/data/sppattern_Hades-MIROC/N30_90/
