...
! % dclfrt draw_bar_test.f90 -g && ./a.out
!--------------------------------------------------------------
...
!      REAL, PARAMETER    :: nmax = 50.0                             ! 度数分布表 横軸の上限
      REAL, PARAMETER    :: nmax = 10.0                             ! 度数分布表 横軸の上限
      INTEGER, PARAMETER :: ndata = 5                              ! データ総数
      REAL, PARAMETER    :: dmin = -1.0                            ! 空間相関の下限
      REAL, PARAMETER    :: dmax = 1.0                             ! 空間相関の上限
      REAL    :: patn(ndata) = (/ -0.5,1.0,0.7,0.7,0.6 /)          ! 空間相関
      INTEGER :: n
...
!! 出力ファイル名を指定
      CHARACTER ( LEN = 200 ) :: OFILE = 'sppatern_test'       ! output file name
      CHARACTER( LEN = * ), PARAMETER :: title = "sppatern_test"
