...
# HadCRUT5 アンサンブル平均とMIROC アンサンブル平均&run1~50の面積重み月領域平均値の度数分布表を描画するプログラム
...
LTOP=/home/honoka/work/HadCRUT5_MIROC6/trendwgtav/
LTOP2=/home/honoka/work/HadCRUT5_MIROC6/trendwgtav/data/${area}
cd ${LTOP2}                                                        # データの在り処に移動

## 面積重み付き領域平均値の度数分布表をDCLで描画
cat << EOF > draw_bar_Hadav_MIROCav.f90
...
!      REAL, PARAMETER    :: nmax = 40.0                             ! 度数分布表 横軸の上限
      ...
      REAL, PARAMETER    :: dmax = 1.0                              ! 領域平均値の上限 
...
!      INTEGER, PARAMETER :: nd = 21                                ! -0.00005, 0, 0.00005, ... , 0.00095, 0.0001の21本の棒 (N30_90の場合)
      INTEGER, PARAMETER :: nd = 10                                 ! 0.0, 0.1, ... , 0.9, 1.0の10本の棒 (N30_90の場合)
      REAL               :: x(nd)
      REAL               :: y(nd)
      REAL               :: x1(nd)                                  ! 棒の両端の座標値
      REAL               :: x2(nd)                                  ! 棒の両端の座標値
...
!! 入力ファイル名を指定
      CHARACTER ( LEN = 200 ) :: IFILE = 'HadCRUT5_1_200av_trendwgtav_1960_2014_${mon}_${area}.txt'     ! input file name (trend : 面積の重み付けあり)
      CHARACTER ( LEN = 200 ) :: IFILE2 = 'MIROC6r1_50av_trendwgtav_1960_2014_${mon}_${area}.txt'       ! input file name (trend : 面積の重み付けあり)
      CHARACTER ( LEN = 200 ) :: IFILE3 = 'MIROC6_r1-50_trendwgtav_1960_2014_${mon}_${area}.txt'        ! input file name (trend : 面積の重み付けあり)

!! 出力ファイル名を指定
      CHARACTER ( LEN = 200 ) :: OFILE = 'bar_trendwgtav_Hadav-MIROC_${mon}_${area}'              ! output file name (trend : 面積の重み付けあり)
      CHARACTER( LEN = * ), PARAMETER :: title = "${mon}"                                         ! 図のタイトル (trend : 面積の重み付けあり)
...
! 座標軸
! x軸
      CALL UXAXDV( 'B', 0.1, 0.2 )                  ! DXAXDV( '場所', 小目盛, 大目盛 )
      CALL UXAXDV( 'T', 0.1, 0.2 )
! y軸
...
# 画像の切り取り (convert)
convert -density 100 -crop 498x525+166+75 bar_trendwgtav_Hadav-MIROC_${mon}_${area} bar_trendwgtav_Hadav-MIROC_${mon}_${area}_ct

mv ./bar_trendwgtav_Hadav-MIROC_${mon}_${area} ./bar/     # trend : 面積の重み付けあり
mv ./bar_trendwgtav_Hadav-MIROC_${mon}_${area}_ct ./bar/     # trend : 面積の重み付けあり
mv ./draw_bar_Hadav_MIROCav.f90 ${LTOP}/src/
