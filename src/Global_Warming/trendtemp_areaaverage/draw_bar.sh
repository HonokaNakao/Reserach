...
# HadCRUT5 ensemble meanとMIROC ensemble mean&run1~50の面積重み月領域平均値の度数分布表を描画するプログラム
...
!! 入力ファイル名を指定
      CHARACTER ( LEN = 200 ) :: IFILE = 'HadCRUT5es_trendwgtav_1960_2014_${mon}_${area}.txt'     ! input file name (trend : 面積の重み付けあり)
      CHARACTER ( LEN = 200 ) :: IFILE2 = 'MIROC6es_trendwgtav_1960_2014_${mon}_${area}.txt'      ! input file name (trend : 面積の重み付けあり)
      CHARACTER ( LEN = 200 ) :: IFILE3 = 'MIROC6_r1-50_trendwgtav_1960_2014_${mon}_${area}.txt'  ! input file name (trend : 面積の重み付けあり)

!! 出力ファイル名を指定
      CHARACTER ( LEN = 200 ) :: OFILE = 'bar_trendwgtav_Hades-MIROC_${mon}_${area}'              ! output file name (trend : 面積の重み付けあり)
      CHARACTER( LEN = * ), PARAMETER :: title = "Hades-MIROC_${area} ${mon}"                     ! 図のタイトル (trend : 面積の重み付けあり)

!---------------------------------------------------------------
!! 面積重み付き領域平均値データの読み込み
! HadCRUT5 ensemble meanの領域平均値
      OPEN ( 10, FILE=IFILE, STATUS='OLD' )    ! open input file
      READ( 10, * ) HadCRUTestrendav
      CLOSE ( 10 )

! MIROC ensemble meanの領域平均値
...
#mv ./bar_sppattern_Hades-MIROC_${mon}_${area} ./bar/     # trend : 面積の重み付けなし
mv ./bar_trendwgtav_Hades-MIROC_${mon}_${area} ./bar/     # trend : 面積の重み付けあり
mv ./draw_bar.f90 ${LTOP}/src/
