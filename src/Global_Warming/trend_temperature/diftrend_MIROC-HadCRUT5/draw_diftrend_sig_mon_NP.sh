...
cat << EOF > draw_diftrend_sig_mon_NP.f90
PROGRAM DRAW_DifTrend_Sig95_90_NP

! トレンドの差(diftrend)と有意性(sig95_90)をDCLで重ね描きするプログラム
! 1960~2014年1月
!
! Compile this program with the following way ;
! % dclfrt draw_diftrend_sig.f90 -fconvert=big-endian && ./a.out
!--------------------------------------------------------------
...
!! 出力ファイル名を指定               123456789-123456789-123456789-123456789-123456789-
      CHARACTER ( LEN = 200 ) :: OFILE = 'diftrend_sig9590_MIROCes-Hades_${yr}_${mon}_NP'    ! output file name
...
END PROGRAM DRAW_DifTrend_Sig95_90_NP
EOF

dclfrt draw_diftrend_sig_mon_NP.f90 && ./a.out                                     # DCLで描画する際は、little-endianでコンパイルする(draw_trend.sig.f90内で、gtoolのみbig-endian指定する) 

rm a.out GTAXLOC* 
mv ./draw_diftrend_sig_mon_NP.f90 /home/honoka/work/HadCRUT5_MIROC6/diftrend/src/  # fortranファイルをsrcに移動

mkdir ${LTOP}/NP/diftrend_MIROC-Hades_${yr}_${mon}
mv ./diftrend_sig9590_MIROCes-Hades_${yr}_${mon}_NP ${LTOP}/NP/diftrend_MIROC-Hades_${yr}_${mon}/
