...
# diftrend = MIROC6 run1-50 trend - HadCRUT5 ensemble mean trend

# (1)特定の期間のtrend、sig95_90_trend(GTOOLデータファイル)を読む
#    (例)diftrend_MIROC6.1-Hades_1960_2014_1 -> 1960年-2014年1月のトレンドの差
#        MIROC6_sig95_90_trend.1_1960_2014_1_mon -> 1960年-2014年１月の有意性(90,95%)のデータ
#
...
DTOP='MIROC6_T2_run1_50_ensemble_1960_2014'
...
cat << EOF > draw_diftrend_sig_r1_50_mon_NP.f90
PROGRAM DRAW_DifTrend_Sig95_90_r1_50_mon_NP

! トレンド(trend)と有意性(sig95_90)をDCLで重ね描きするプログラム (北極点中心に、北緯20-90度域を描画)
! 1960~2014年1月
!
! Compile this program with the following way ;
! % dclfrt draw_diftrend_sig_r1_50_mon_NP.f90 -fconvert=big-endian && ./a.out
!--------------------------------------------------------------
...
!! 入力ファイル名を指定               123456789-123456789-123456789-123456789-123456789-1
      ...
      CHARACTER ( LEN = 200 ) :: IFILE3 = 'diftrend_MIROC6.${runnum}-Hades_${yr}_${mon}'             ! input file name (1960~2014年 月毎のトレンドの差)
      CHARACTER ( LEN = 200 ) :: IFILE4 = '${strnm}_sig95_90_trend.${runnum}_${yr}_${mon}_mon'       ! input file name (1960~2014年 全期間の有意性：有意水準10%,5%)

!! 出力ファイル名を指定               123456789-123456789-123456789-123456789-123456789-
      CHARACTER ( LEN = 200 ) :: OFILE = 'diftrend_sig9590.MIROC${runnum}-Hades_${yr}_${mon}_NP'      ! output file name

      character( len = * ), parameter :: title = "diftrend/sig95_90 r${runnum} ${mon}"
...
!
      ...
      REAL :: diftrendout( lon+1, lat )     ! トレンド (回帰係数):DCLで描画するtrend
...
!      REAL, PARAMETER :: tlev1(13) = (/ -10.0,-1.0,-0.6,-0.4,-0.2,-0.1,0.0,0.1,0.2,0.4,0.6,1.0,10.0 /)     ! 軸(tone level)の値 (trendout)
      REAL, PARAMETER :: tlev1(13) = (/ -5.0,-2.0,-1.0,-0.5,-0.2,-0.1,0.0,0.1,0.2,0.5,1.0,2.0,5.0 /)        ! 軸(tone level)の値 (trendout)
...
!  call sgstxy(lonmin, lonmax, latmin, latmax)   ! set lon & lat window (緯度・経度を指定)
  call sgstxy(lonmin, lonmax, 20., latmax)   ! set lon & lat window (緯度・経度を指定)
  CALL GRSTRN(31)                               ! 正射図法
...
END PROGRAM DRAW_DifTrend_Sig95_90_r1_50_mon_NP
EOF

dclfrt draw_diftrend_sig_r1_50_mon_NP.f90 && ./a.out                                               # DCLで描画する際は、little-endianでコンパイルする(draw_trend.sig.f90内で、gtoolのみbig-endian指定する) 
rm a.out GTAXLOC* 
mv ./draw_diftrend_sig_r1_50_mon_NP.f90 /home/honoka/work/HadCRUT5_MIROC6/diftrend/src/            # fortranファイルをsrcに移動 
mv ./diftrend_sig9590.MIROC${runnum}-Hades_${yr}_${mon}_NP ${LTOP}/NP/diftrend_MIROC-Hades_${yr}_${mon}/  # 出力ファイルをdataに移動

# 画像の切り取り (convert)
#convert -density 100 -crop 664x470+110+40 ${strnm}_trend_sig9590.${runnum}_${yr}_${mon}_mon_NP ${strnm}_trend_sig9590.${runnum}_${mon}_NP
