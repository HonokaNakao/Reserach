...
cat << EOF > draw_trend_sig_r1_50_mon_NP.f90
PROGRAM DRAW_Trend_Sig95_90_r1_50_mon_NP

! トレンド(trend)と有意性(sig95_90)をDCLで重ね描きするプログラム (北極点中心に、北緯20-90度域を描画)
! 1960~2014年1月
!
! Compile this program with the following way ;
! % dclfrt draw_trend_sig_r1_50_mon_NP.f90 -fconvert=big-endian && ./a.out
!--------------------------------------------------------------
...
!! 入力ファイル名を指定               123456789-123456789-123456789-123456789-123456789-1
      ...
      CHARACTER ( LEN = 200 ) :: IFILE3 = '${strnm}_trend.${runnum}_${yr}_${mon}_mon'             ! input file name (1960~2014年 月毎のトレンド)
...
!! 出力ファイル名を指定               123456789-123456789-123456789-123456789-123456789-
      CHARACTER ( LEN = 200 ) :: OFILE = '${strnm}_trend_sig9590.${runnum}_${yr}_${mon}_mon_NP'       ! output file name

      character( len = * ), parameter :: title = "${strnm} trend/sig95_90 1960-2014 ${runnum}_${mon}"
...
END PROGRAM DRAW_Trend_Sig95_90_r1_50_mon_NP
EOF

dclfrt draw_trend_sig_r1_50_mon_NP.f90 && ./a.out                            # DCLで描画する際は、little-endianでコンパイルする(draw_trend.sig.f90内で、gtoolのみbig-endian指定する) 
rm a.out GTAXLOC* 
mv ./draw_trend_sig_r1_50_mon_NP.f90 /home/honoka/work/MIROC6/src/            # fortranファイルをsrcに移動 
