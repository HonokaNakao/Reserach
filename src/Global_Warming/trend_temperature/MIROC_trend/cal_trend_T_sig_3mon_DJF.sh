...
#    (例) MIROC6_T2_run1_anomalies_1960_2014_120102 -> 1960年-2014年DJFのデータ
#
...
strmon=12                         # strmon=3(MAM), 6(JJA), 9(SON), 12(DJF)
var='MIROC6_T2'
mon3='120102'                     # MAM(030405), JJA(060708), SON(091011) -> YMAX=55, MMAX=165 OR # DJF(120102) -> YMAX=54, MMAX=162に設定

DTOP=${var}_run${runnum}_anomalies_${stryr}_${endyr}

cd ${LTOP}/data/run${runnum}/${var}_run${runnum}_${stryr}_${endyr}_${mon3}/    # GTOOLの在り処に移動

## 月別データを読み込む & トレンド計算 & 検定統計量T計算
cat << EOF > cal_trend_T_sig_3mon_DJF.f90
PROGRAM cal_trend_T_sig_3mon_DJF

! gfortran -fconvert=big-endian cal_trend_T_sig_3mon_DJF.f90
...
!! 入力ファイル名を指定               123456789-123456789-123456789-123456789-123456789-1 
  CHARACTER ( LEN = 200 ) :: IFILE = '${DTOP}_${mon3}' ! input file name

!! 出力ファイル名を指定               123456789-123456789-123456789-123456789-123456789- 
  CHARACTER ( LEN = 200 ) :: OFILE = 'MIROC6_trend.${runnum}_1960_2014_${mon3}_mon'             ! output file name
  CHARACTER ( LEN = 200 ) :: OFILE2 = 'MIROC6_T_trend.${runnum}_1960_2014_${mon3}_mon'        ! output file name
  CHARACTER ( LEN = 200 ) :: OFILE3 = 'MIROC6_tval90_trend.${runnum}_1960_2014_${mon3}_mon'   ! output file name
  CHARACTER ( LEN = 200 ) :: OFILE4 = 'MIROC6_sig90_trend.${runnum}_1960_2014_${mon3}_mon'    ! output file name
  CHARACTER ( LEN = 200 ) :: OFILE5 = 'MIROC6_tval95_trend.${runnum}_1960_2014_${mon3}_mon'   ! output file name
  CHARACTER ( LEN = 200 ) :: OFILE6 = 'MIROC6_sig95_trend.${runnum}_1960_2014_${mon3}_mon'    ! output file name
  CHARACTER ( LEN = 200 ) :: OFILE7 = 'MIROC6_sig95_90_trend.${runnum}_1960_2014_${mon3}_mon'   ! output file name
!  CHARACTER ( LEN = 200 ) :: OFILE8 = 'MIROC6_sigmae_trend.${runnum}_1960_2014_${mon3}_mon'   ! output file name
!  CHARACTER ( LEN = 200 ) :: OFILE9 = 'MIROC6_esty_trend.${runnum}_1960_2014_${mon3}_mon'     ! output file name
  ...
  DO i = 1, IMAX
     DO j = 1, JMAX
        IF( ndata(i,j) .NE. 0.0000 .AND. block4(i,j) .NE. 0.0000 ) THEN
                trend(i,j) = 120.0*( block1(i,j) - ndata(i,j)*avex(i,j)*avey(i,j) ) / (block4(i,j))  ! 10*[K/yr] -> [K/decade]
        ELSE
                trend(i,j) = vmiss
        END IF
     END DO
...
gfortran -fconvert=big-endian ./cal_trend_T_sig_3mon_DJF.f90;./a.out > ${LTOP}/log/test_ver3.txt           # コンパイル & 実行 & 実行結果を確認 (redirection)
mv ./cal_trend_T_sig_3mon_DJF.f90 ${LTOP}/src/                                                            # fortranファイルをsrcに移動
...
# trend
gtcont MIROC6_trend.${runnum}_1960_2014_${mon3}_mon tone=-1,-0.6,-0.4,-0.2,-0.1,0.0,0.1,0.2,0.4,0.6,1.0 pat=10999,22999,28999,48999,53999,58999,65999,70999,89999,99999 map=1 -nocnt -print ps:MIROC6_trend.${runnum}_1960_2014_${mon3}_mon_rg-1_1.ps
gtcont MIROC6_trend.${runnum}_1960_2014_${mon3}_mon tone=-10.0,-1.0,-0.6,-0.4,-0.2,-0.1,0.0,0.1,0.2,0.4,0.6,1.0,10.0 pat=10999,12999,22999,28999,48999,53999,58999,65999,70999,89999,97999,99999 map=1 -nocnt -print ps:MIROC6_trend.${runnum}_1960_2014_${mon3}_mon_rg-10_10.ps
# sig95_90
gtcont MIROC6_sig95_90_trend.${runnum}_1960_2014_${mon3}_mon tone=85.0,90.0,95.0,100.0 pat=10999,53999,89999 map=1 -nocnt -print ps:MIROC6_sig95_90_trend.${runnum}_1960_2014_${mon3}_mon.ps
rm colormap.x11

# psファイルからpngファイルへ形式変更
convert -density 100 -rotate 90 MIROC6_trend.${runnum}_1960_2014_${mon3}_mon_rg-1_1.ps MIROC6_trend.${runnum}_1960_2014_${mon3}_mon_rg-1_1.png
convert -density 100 -rotate 90 MIROC6_trend.${runnum}_1960_2014_${mon3}_mon_rg-10_10.ps MIROC6_trend.${runnum}_1960_2014_${mon3}_mon_rg-10_10.png

convert -density 100 -rotate 90 MIROC6_sig95_90_trend.${runnum}_1960_2014_${mon3}_mon.ps MIROC6_sig95_90_trend.${runnum}_1960_2014_${mon3}_mon.png
