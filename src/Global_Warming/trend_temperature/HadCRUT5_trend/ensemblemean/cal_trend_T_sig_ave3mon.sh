...
mon3='120102'                                         # MAM(030405), JJA(060708), SON(091011) -> YMAX=55 OR # DJF(120102) -> YMAX=54
#strmon=$1
#strmon=1
...
cat << EOF > cal_trend_T_sig_ave3mon.f90
PROGRAM cal_trend_T_sig_ave3mon

! gfortran -fconvert=big-endian cal_trend_T_sig_ave3mon.f90
...
!  INTEGER, PARAMETER :: MMAX= 165   ! No. of months (1960-2014の年55年×3ヶ月=165個の月別データ, MAM.JJA.SON)
  ...
  REAL :: TASM( IMAX, JMAX, YMAX )   ! data(ydata)
  REAL :: x( IMAX, JMAX, YMAX )      ! xdata
  ...
  REAL :: esty( IMAX, JMAX, YMAX )   ! y値の推定値　
...
!! 入力ファイル名を指定               123456789-123456789-123456789-123456789-123456789-1 
  CHARACTER ( LEN = 200 ) :: IFILE = 'HadCRUT.5.0.1.0.analysis.anomalies.ensemble_mean_1960_2014_ave${mon3}' ! input file name

!! 出力ファイル名を指定               123456789-123456789-123456789-123456789-123456789- 
  CHARACTER ( LEN = 200 ) :: OFILE = 'trend_1960_2014_ave${mon3}'             ! output file name
  CHARACTER ( LEN = 200 ) :: OFILE2 = 'T_trend_1960_2014_ave${mon3}'        ! output file name
  CHARACTER ( LEN = 200 ) :: OFILE3 = 'tval90_trend_1960_2014_ave${mon3}'   ! output file name
  CHARACTER ( LEN = 200 ) :: OFILE4 = 'sig90_trend_1960_2014_ave${mon3}'    ! output file name
  CHARACTER ( LEN = 200 ) :: OFILE5 = 'tval95_trend_1960_2014_ave${mon3}'   ! output file name
  CHARACTER ( LEN = 200 ) :: OFILE6 = 'sig95_trend_1960_2014_ave${mon3}'    ! output file name
  CHARACTER ( LEN = 200 ) :: OFILE7 = 'sig95_90_trend_1960_2014_ave${mon3}'   ! output file name
!  CHARACTER ( LEN = 200 ) :: OFILE8 = 'sigmae_trend_1960_2014_ave${mon3}'   ! output file name
!  CHARACTER ( LEN = 200 ) :: OFILE9 = 'esty_trend_1960_2014_ave${mon3}'     ! output file name
...
!! trend計算の分子１項目(block1) sigma(xi*yi)
  ...
  DO i = 1, IMAX
     DO j = 1, JMAX
        DO y = 1, YMAX
           IF ( TASM(i,j,y) .NE. vmiss ) THEN
              xx(i,j) = xx(i,j) + (x(i,j,y))**2
              block1(i,j) = block1(i,j) + x(i,j,y)*TASM(i,j,y)
           END IF
        END DO
      END DO
  END DO
...
!! yの推定値 (esty) 
  DO i = 1, IMAX
     DO j = 1, JMAX
        DO y = 1, YMAX
           IF( TASM(i,j,y) .NE. vmiss ) THEN
             esty(i,j,y) = trend(i,j)*x(i,j,y) + b(i,j)
           END IF
        END DO 
     END DO 
  END DO 
...
!! T計算の分子 block3(i,j) = sigma((x-avex)**2) )
  ...
  DO i = 1, IMAX
     DO j = 1, JMAX
        DO y = 1, YMAX
           IF( TASM(i,j,y) .NE. vmiss ) THEN
             block2(i,j) = block2(i,j) + ( TASM(i,j,y) - esty(i,j,y) ) ** 2
...
gfortran -fconvert=big-endian ./cal_trend_T_sig_ave3mon.f90;./a.out > ${LTOP}/log/test_ver3.txt    # コンパイル & 実行 & 実行結果を確認 (redirection)

mv ./cal_trend_T_sig_ave3mon.f90 ${LTOP}/src/t_test/t_test_ensemblemean/                           # fortranファイルをsrcに移動           

## 描画
cd ${LTOP}/data/data_ensemble_monthly/${DTOP}_1960_2014/${DTOP}_1960_2014_${mon3}                              # GTOOLの在り処に移動
gtcont trend_1960_2014_ave${mon3}_mon map=1 color=50 -nocnt -print ps:trend_1960_2014_ave${mon3}_mon.ps
...
gtcont trend_1960_2014_ave${mon3} tone=-1,-0.6,-0.4,-0.2,-0.1,0.0,0.1,0.2,0.4,0.6,1.0 pat=10999,22999,28999,48999,53999,58999,65999,70999,89999,99999 map=1 -nocnt -print ps:trend_1960_2014_ave${mon3}_rg-1_1.ps
...
gtcont sig95_90_trend_1960_2014_ave${mon3} tone=85.0,90.0,95.0,100.0 pat=10999,53999,89999 map=1 -nocnt -print ps:sig95_90_trend_1960_2014_ave${mon3}.ps
rm colormap.x11
