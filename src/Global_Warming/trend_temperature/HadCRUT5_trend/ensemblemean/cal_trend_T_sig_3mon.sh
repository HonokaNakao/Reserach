...
mon3='120102'                                         # MAM(030405), JJA(060708), SON(091011) -> YMAX=55, MMAX=165 OR # DJF(120102) -> YMAX=54, MMAX=162に設定
#strmon=$1
#strmon=1

cd ${LTOP}/data/data_ensemble_monthly/${DTOP}_1960_2014/${DTOP}_1960_2014_${mon3}/           # GTOOLの在り処に移動

## 月別データを読み込む & トレンド計算 & 検定統計量T計算
cat << EOF > cal_trend_T_sig_3mon.f90
PROGRAM cal_trend_T_sig_3mon

! gfortran -fconvert=big-endian cal_trend_T_sig_3mon.f90
  ...
  INTEGER, PARAMETER :: YMAX= 54    ! No. of years (1961-2014年=54年, DJFの場合)
!  INTEGER, PARAMETER :: YMAX= 55    ! No. of years (1960-2014年=55年, MAM,JJA,SONの場合)

!  INTEGER, PARAMETER :: MMAX= 660   ! No. of months (1960-2014年660個の月別データ)
!  INTEGER, PARAMETER :: MMAX= 55   ! No. of months (1960-2014年1月55個の月別データ)
  INTEGER, PARAMETER :: MMAX= 162   ! No. of months (1960-2014の年54年×3ヶ月=162個の月別データ, DJF)
!  INTEGER, PARAMETER :: MMAX= 165   ! No. of months (1960-2014の年55年×3ヶ月=165個の月別データ, MAM.JJA.SON)
  ...
  REAL :: sig95_90( IMAX, JMAX )     ! 有意水準5%の有意性検定 (有意:trend(i,j) 有意水準10%~5%未満の地点:-99 有意でない:vmiss)

!! 入力ファイル名を指定               123456789-123456789-123456789-123456789-123456789-1 
  CHARACTER ( LEN = 200 ) :: IFILE = 'HadCRUT.5.0.1.0.analysis.anomalies.ensemble_mean_1960_2014_${mon3}' ! input file name

!! 出力ファイル名を指定               123456789-123456789-123456789-123456789-123456789- 
  CHARACTER ( LEN = 200 ) :: OFILE = 'trend_1960_2014_${mon3}_mon'             ! output file name
  CHARACTER ( LEN = 200 ) :: OFILE2 = 'T_trend_1960_2014_${mon3}_mon'        ! output file name
  CHARACTER ( LEN = 200 ) :: OFILE3 = 'tval90_trend_1960_2014_${mon3}_mon'   ! output file name
  CHARACTER ( LEN = 200 ) :: OFILE4 = 'sig90_trend_1960_2014_${mon3}_mon'    ! output file name
  CHARACTER ( LEN = 200 ) :: OFILE5 = 'tval95_trend_1960_2014_${mon3}_mon'   ! output file name
  CHARACTER ( LEN = 200 ) :: OFILE6 = 'sig95_trend_1960_2014_${mon3}_mon'    ! output file name
  CHARACTER ( LEN = 200 ) :: OFILE7 = 'sig95_90_trend_1960_2014_${mon3}_mon'   ! output file name
!  CHARACTER ( LEN = 200 ) :: OFILE8 = 'sigmae_trend_1960_2014_${mon3}_mon'   ! output file name
!  CHARACTER ( LEN = 200 ) :: OFILE9 = 'esty_trend_1960_2014_${mon3}_mon'     ! output file name
...
!! トレンド計算
  trend = 0.0
!  trend(:,:) = 120.0*( block1(:,:) - ndata(:,:)*avex(:,:)*avey(:,:) ) / (xx(:,:) - ndata(:,:)*avex(:,:)*avex(:,:))  ! 120*[K/month] -> [K/decade] (seasonal trend)
!  trend(:,:) = 10.0*( block1(:,:) - ndata(:,:)*avex(:,:)*avey(:,:) ) / (xx(:,:) - ndata(:,:)*avex(:,:)*avex(:,:))  ! 10*[K/yr] -> [K/decade]      (monthly trend)

  DO i = 1, IMAX
     DO j = 1, JMAX
        IF( ndata(i,j) .NE. 0.0000 .AND. block4(i,j) .NE. 0.0000 ) THEN
                trend(i,j) = 120.0*( block1(i,j) - ndata(i,j)*avex(i,j)*avey(i,j) ) / (block4(i,j))  ! 120*[K/yr] -> [K/decade]
        ELSE
                trend(i,j) = vmiss
        END IF
     END DO
...
!! 検定統計量T値
  ...
  HEAD( 3 ) = 'test statistic T'              ! change item
  WRITE( 30 ) HEAD
  WRITE( 30 ) T
...
gfortran -fconvert=big-endian ./cal_trend_T_sig_3mon.f90;./a.out > ${LTOP}/log/test_ver3.txt    # コンパイル & 実行 & 実行結果を確認 (redirection)

mv ./cal_trend_T_sig_3mon.f90 ${LTOP}/src/t_test/t_test_ensemblemean/                           # fortranファイルをsrcに移動           

## 描画
cd ${LTOP}/data/data_ensemble_monthly/${DTOP}_1960_2014/${DTOP}_1960_2014_${mon3}                              # GTOOLの在り処に移動
#gtcont trend_1960_2014_${mon3}_mon map=1 color=50 -nocnt -print ps:trend_1960_2014_${mon3}_mon.ps
...
gtcont trend_1960_2014_${mon3}_mon tone=-1.0,-0.6,-0.4,-0.2,-0.1,0.0,0.1,0.2,0.4,0.6,1.0 pat=10999,22999,28999,48999,53999,58999,65999,70999,89999,99999 map=1 -nocnt -print ps:trend_1960_2014_${mon3}_mon_rg-1_1.ps
gtcont trend_1960_2014_${mon3}_mon tone=-10.0,-1,-0.6,-0.4,-0.2,-0.1,0.0,0.1,0.2,0.4,0.6,1.0,10.0 pat=10999,12999,22999,28999,48999,53999,58999,65999,70999,89999,97999,99999 map=1 -nocnt -print ps:trend_1960_2014_${mon3}_mon_rg-10_10.ps
#gtcont sig90_trend_1960_2014_${strmon}_mon tone=-1,-0.6,-0.4,-0.2,-0.1,0.0,0.1,0.2,0.4,0.6,1.0 pat=10999,22999,28999,48999,53999,58999,65999,70999,89999,99999 map=1 -nocnt -print ps:sig90_trend_1960_2014_${strmon}_mon_rg-1_1.ps
#gtcont sig95_trend_1960_2014_${strmon}_mon tone=-1,-0.6,-0.4,-0.2,-0.1,0.0,0.1,0.2,0.4,0.6,1.0 pat=10999,22999,28999,48999,53999,58999,65999,70999,89999,99999 map=1 -nocnt -print ps:sig95_trend_1960_2014_${strmon}_mon_rg-1_1.ps
gtcont sig95_90_trend_1960_2014_${mon3}_mon tone=85.0,90.0,95.0,100.0 pat=10999,53999,89999 map=1 -nocnt -print ps:sig95_90_trend_1960_2014_${mon3}_mon.ps
rm colormap.x11

# psファイルからpngファイルへ形式変更
convert -density 100 -rotate 90 trend_1960_2014_${mon3}_mon_rg-1_1.ps trend_1960_2014_${mon3}_mon_rg-1_1.png
convert -density 100 -rotate 90 trend_1960_2014_${mon3}_mon_rg-10_10.ps trend_1960_2014_${mon3}_mon_rg-10_10.png
convert -density 100 -rotate 90 sig95_90_trend_1960_2014_${mon3}_mon.ps sig95_90_trend_1960_2014_${mon3}_mon.png
