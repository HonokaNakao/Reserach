...
#cd ${LTOP}/data/data_ensemble_monthly/${DTOP}_1960_2014/                                        # GTOOLの在り処に移動 (1960~2014年全期間のトレンド)
cd ${LTOP}/data/data_ensemble_monthly/${DTOP}_1960_2014/${DTOP}_1960_2014_${strmon}/           # GTOOLの在り処に移動 (1~12月 月毎のトレンド)
...
cat << EOF > draw_trend_sig_mon_NP.f90
PROGRAM DRAW_Trend_Sig95_90_NP
...
!      CHARACTER ( LEN = 200 ) :: IFILE3 = 'trend_1960_2014_mon'                    ! input file name (1960~2014年 全期間のトレンド)
      CHARACTER ( LEN = 200 ) :: IFILE3 = 'trend_1960_2014_${strmon}_mon'          ! input file name (1960~2014年 月毎のトレンド)
!      CHARACTER ( LEN = 200 ) :: IFILE4 = 'sig95_90_trend_1960_2014_mon'           ! input file name (1960~2014年 月毎の有意性：有意水準10%,5%)
      CHARACTER ( LEN = 200 ) :: IFILE4 = 'sig95_90_trend_1960_2014_${strmon}_mon'  ! input file name (1960~2014年 全期間の有意性：有意水準10%,5%)
...
!      CHARACTER ( LEN = 200 ) :: OFILE = 'trend_sig9590_1960_2014_${strmon}_mon'  ! output file name
      CHARACTER ( LEN = 200 ) :: OFILE = 'trend_sig9590_1960_2014_${strmon}_mon_NP'  ! output file name

      character( len = * ), parameter :: title = "${strnm} trend/sig95_90 1960-2014 ${strmon}"
...
!  call sgsmpl(0., 90., 0.)                     ! rotation angle (中心点の設定。東経150度、北緯60度を中心とし0度回転。 ex:北極(0.,90.,0.))
...
END PROGRAM DRAW_Trend_Sig95_90_NP
EOF

dclfrt draw_trend_sig_mon_NP.f90 && ./a.out                            # DCLで描画する際は、little-endianでコンパイルする(draw_trend.sig.f90内で、gtoolのみbig-endian指定する) 

rm a.out GTAXLOC* 
mv ./draw_trend_sig_mon_NP.f90 ${LTOP}/src/t_test/t_test_ensemblemean/                           # fortranファイルをsrcに移動 

# 画像の切り取り (convert)
convert -density 100 -crop 664x470+110+40 trend_sig9590_1960_2014_${strmon}_mon_NP trend_sig9590_${strmon}_NP
