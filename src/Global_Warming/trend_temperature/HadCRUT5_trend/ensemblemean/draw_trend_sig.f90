...
!! 入力ファイル名を指定               123456789-123456789-123456789-123456789-123456789-1
      ...
      CHARACTER ( LEN = 200 ) :: IFILE3 = 'trend_1960_2014_5_mon'           ! input file name (トレンド)
      CHARACTER ( LEN = 200 ) :: IFILE4 = 'sig95_90_trend_1960_2014_5_mon'  ! input file name (有意性：有意水準10%,5%)

!! 出力ファイル名を指定               123456789-123456789-123456789-123456789-123456789-
      CHARACTER ( LEN = 200 ) :: OFILE = 'trend_sig9590_1960_2014_5_mon'  ! output file name

      character( len = * ), parameter :: title = "trend/sig95_90 1960-2014 5"
...
!  INTEGER :: ipat(noc)
      ...
      INTEGER, PARAMETER :: ipat2(3) = (/ 1452, 1413, 1472 /)                                             ! 色(color pattern)の値(sig95_90out) (色,パターンの種類,太さ&大きさ,密度)
...
!! DCLで描画
  ...
  call glrset('rmiss', -9999.)                  ! put number as non available (欠損値の値を定義。rmiss=-9999.)
...
!!! トーンバー
!  call tone_bar_eq(tmin, tmax, noc, inum, dt, bmin, bmax, unit)
!  call tone_bar_eq(tmin, tmax, noc, inum, dt, bmin, bmax, unit, tlev1)
...
!  DO i = 1,3
!     y(1) = 0. ; y(2) = 0. ; y(3) = 1. ; y(4) = 1.
!     x(i) = tlev1(i) ; x(i+1) = tlev1(i+1) ; x(i+2) = tlev1(i+1) ; x(i+3) = tlev1(i)
!     CALL SGTNZU( 4, x, y, ipat1(i) )
!  END DO
!?-----------
...
!---------------------------------------------
!
!subroutine tone_bar_eq(tmin, tmax, noc, inum, dt, bmin, bmax, unit)
subroutine tone_bar_eq(tmin, tmax, noc, inum, dt, bmin, bmax, unit, ipat, tlev)
...
!  INCLUDE '/usr/include/netcdf.inc'
  ...
  real :: tmin, tmax
  real :: bar(noc+1,2)
  integer :: inum(noc), ipat(noc)
  real :: dt
  character(len=*) :: bmin, bmax, unit
!?
!  real :: bar(noc,2)
  real :: tlev(noc)
!?
  ...
  do i = 1, noc+1
     bar(i, 1) = tmin + real(i-1)*dt
     bar(i, 2) = tmin + real(i-1)*dt
  end do
...
!? 
  do i = 1, noc
     ...
     bar(noc+1,:) = tlev(1) 
!?
  call udlset('LMSG', .false.)              ! true write lavel message
  call uetone(bar, noc+1, noc+1, 2)         ! tone bar

  call slpvpr(3)                            ! outside frame of tone bar
  call uzlset('LABELYR', .false.)           ! true => write lavel year
  call uzfact(0.8)
  call uysfmt('I3')
  call uyaxdv('R', dt, 10*dt)
